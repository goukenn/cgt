...
final class IGKJS_HorizontalPane extends IGKObject
{
	...
	public function __construct($target)
	{
		...
		//igk_app()->Doc->addTempStyle(dirname(__FILE__)."/Styles/default.pcss");
...
$pane->RenderAJX();
		...
		switch(strtolower($type ))
		{
			...
			case "image/png":
				...
				//store template
				igk_io_save_file_as_utf8_wbom($dir."/".basename($s).".phtml", $div->Render(null) , true);
				break;
			default:
...
$target->Load( igk_html_databinding_treatresponse($s,null, null,null));
	...
	public function loadData($data){
		...
		foreach($data->getElementsByTagName("page") as $e){
			...
			$c = igk_add_article_options(null, $p, $file);
			if ($c){
