...
>
	...
	<xsl:variable name="detailuri" select="'%detailuri%'" ></xsl:variable>
	...
	<xsl:variable name="serverpagination" select="1" ></xsl:variable>
<xsl:template match="/"> 
	<div>Total: <xsl:value-of select="/offres/@count" ></xsl:value-of> </div>
	<div class="view-z igk-row">
		...
		<div style="padding: 10px 4px" class="igk-col igk-col-5-1 igk-col-sm-4-2 igk-col-xsm-3-3 offre">
			<div >
				...
				<a href="{$detailuri}/{current()/@codeCgt}"><h3 class='title'><xsl:value-of select="current()/nom" ></xsl:value-of></h3></a>
				<div>
	...
	<xsl:if test="$serverpagination = 0 ">
...
<div>
	<xsl:comment>Pagination Presentation - Detection</xsl:comment>
